...
def getRlat(a,b,c):
    """Calculate the reciprocal lattice given the direct lattice basis vectors
    """
    return np.cross(b,c)/(np.dot(a,np.cross(b,c))),np.cross(c,a)/(np.dot(b,np.cross(c,a))),np.cross(a,b)/(np.dot(c,np.cross(a,b)))
...
def generateAtomsXYZ(unitcell,atombasis,rmax=30.0):
...
def generateSpinsXYZ(unitcell,atoms=np.array([]),spinOrigin=np.array([0,0,0]),kvec=np.array([0,0,0]),svec=np.array([0,0,1])):
    ...
    """
    astar,bstar,cstar=getRlat(unitcell[0],unitcell[1],unitcell[2])
...
class mPDFcalculator:
    ...
    def makeAtoms(self):
        ...
        self.atoms=generateAtomsXYZ(self.unitcell,self.atombasis,self.rmaxAtoms)
    ...
    def makeSpins(self):
        ...
        """
        self.spins=generateSpinsXYZ(self.unitcell,self.atoms,self.spinOrigin,self.kvec,self.svec)
